...
if __name__ == '__main__':
        ...
        parser.add_argument('--block_size', type=int, default = 81, help="number of layers", required=False)   # previously 57... 54 for moses. 100 for guacamol.
        ...
        #file = open('../gen_csv_again/res.csv', 'w', encoding='utf-8')
        file = open('gen_csv_again/res.csv', 'w', encoding='utf-8')
        csv_writer = csv.writer(file)
        csv_writer.writerow(['Valid molecules', 'unique molecules', 'validity', 'unique', 'novelty'])

        data = pd.read_csv('C:/Users/A/Desktop/molgpt/molgpt/datasets/' + args.data_name + '.csv', engine='python')
        #data = pd.read_csv('/root/workspace/molgpt/datasets/' + args.data_name + '.csv', engine='python')
        ...
        if 'moses' in args.data_name:
            ...
            smiles = data[data['split']!='test']['smiles']
            scaf = data[data['split']!='test']['scaffold_smiles']
        ...
        pad1 = str('6')
        pad2 = str('7')
        content = ' '.join(smiles + scaf + pad + pad1 + pad2)
        ...
        model.load_state_dict(torch.load('C:/Users/A/Desktop/molgpt/molgpt/' + args.model_weight),map_location='cpu')
        #model.to('cuda')
        print('Model loaded')
        ...
        if args.scaffold:
            ...
            # scaf_condition = ['c1ccc(CCN2CCC(Nc3ccccc3)CC2)cc1', 'c1ccc(CN2CCC(Nc3ccccc3)CC2)cc1',
            #                   'c1ccc(NC2CCNCC2)cc1', 'c1ccccc1', 'O=C(C1CCCO1)N(c1ccccc1)C1CCN(CCc2ccccc2)CC1',
            #                   'c1ccc(NC2CCN(CCc3cccs3)CC2)cc1']
        ...
        if prop_condition is None and scaf_condition is None:
            ...
            for i in tqdm(range(gen_iter)):
                    ...
                    sca = None
                    y = sample(model, x, args.block_size, temperature=0.7, sample=True, top_k=None, prop = p, scaffold = sca)   # 0.7 for guacamol
                    for gen_mol in y:
            ...
            for j in scaf_condition:
                ...
                canon_smiles = [canonic_smiles(s) for s in results['smiles']]
                unique_smiles = list(set(canon_smiles))
                print("------------------------unique molecules % = {}".format(len(unique_smiles)))
                if 'moses' in args.data_name:
                        novel_ratio = check_novelty(unique_smiles, set(data[data['split']=='train']['smiles']))   # replace 'source' with 'split' for moses
                else:
                        novel_ratio = check_novelty(unique_smiles, set(data[data['split']=='train']['smiles']))   # replace 'source' with 'split' for moses
                ...
                validity = np.round(len(results)/(args.batch_size*gen_iter), 3)
                unique = np.round(len(unique_smiles)/len(results), 3)
                novelty = np.round(novel_ratio/100, 3)


                csv_writer.writerow([len(molecules),  len(unique_smiles), validity, unique, novelty])
