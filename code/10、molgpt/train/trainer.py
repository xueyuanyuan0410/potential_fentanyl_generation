...
class Trainer():
    ...
    #                     state_dict,
    #                     num_classes):
    #     print(state_dict['head.weight'].size(0))
    #     print(num_classes)
    ...
    def train(self, wandb):
        ...
        def run_epoch(split):
            ...
            for it, (x, y, p, scaffold) in enumerate(loader):
        ...
        for epoch in range(config.max_epochs):
            ...
            if self.config.generate:
                ...
                scaf_condition = [i + str('<') * (40 - len(regex.findall(i))) for i in scaf_condition]

                context = "C"

                for j in scaf_condition:
                    for i in range(2):
                        x = torch.tensor([self.stoi[s] for s in regex.findall(context)], dtype=torch.long)[None,...].repeat(16, 1).to('cuda')
                        p = None
                        # sca = None
                        sca = torch.tensor([self.stoi[s] for s in regex.findall(j)], dtype=torch.long)[None, ...].repeat(16, 1).to('cuda')
