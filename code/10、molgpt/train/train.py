...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain', action='store_true',
                        default=False, help='finetune from pretrained model')
    ...
    data = pd.read_csv('C:/Users/A/Desktop/molgpt/molgpt/datasets/moses2.csv')
    data = data.dropna(axis=0).reset_index(drop=True)
    # data = data.sample(frac = 0.1).reset_index(drop=True)
    data.columns = data.columns.str.lower()

#reset_index是对清洗过后的数据进行重进给索引。
    ...
    if 'moses' in args.data_name:
        ...
        val_data = data[data['split'] == 'test_scaffolds'].reset_index(
            drop=True)   # test for Moses. val for guacamol
    ...
    #对化学文本进行处理时我们希望处理的最小单位粒度是元素或者键，对smiles化学表达式进行分词
    pattern = "(\[[^\]]+]|<|Br?|Cl?|N|O|S|P|F|I|b|c|n|o|s|p|\(|\)|\.|=|#|-|\+|\\\\|\/|:|~|@|\?|>|\*|\$|\%[0-9]{2}|[0-9])"
    regex = re.compile(pattern)

    # a=list(smiles.values)
    # b=list(vsmiles.values)
    # c=a+b
    # for i in (a+b):
    #     d=regex.findall(i.strip())
    #     print(d)
    ...
    #print('max_lens')
    #print(max_len)
    max_len = 81#我尝试输出上面lens的最大长度是54，所以这个地方为什么是81？
    ...
    # for i in smiles:
    #     print(regex.findall(i.strip()))
    #     a = str('<') * (max_len - len(regex.findall(i.strip())))
    #     print(a)
    #     smile = i + a
    #     print(smile)
    ...
    padstring = [str('6'), str('7')]
    whole_string = ' '.join(smiles + vsmiles + scaffold + vscaffold + padstring)
    ...
    valid_dataset = SmileDataset(args, vsmiles, whole_string, max_len, augmentation=0, prop=vprop, aug_prob=0, scaffold=vscaffold, scaffold_maxlen= scaffold_max_len)
    ...
    model = GPT(mconf)
    if args.pretrain:
        state_dict = torch.load('../cond_gpt/weights/scaffold_moses.pt', map_location='cpu')
    ...
    tconf = TrainerConfig(max_epochs=args.max_epochs, batch_size=args.batch_size, learning_rate=args.learning_rate,
                            ...
                            num_workers=0, ckpt_path=f'../cond_gpt/weights/{args.run_name}.pt', block_size=train_dataset.max_len, generate=False)
    trainer = Trainer(model, train_dataset, valid_dataset,
                        tconf, train_dataset.stoi, train_dataset.itos)

    # if args.pretrain:
    #     state_dict = torch.load('../cond_gpt/weights/scaffold_moses.pt', map_location='cpu')
    #     trainer.load_checkpoint(state_dict, train_dataset.vocab_size)
